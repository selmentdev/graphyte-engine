...
namespace Graphyte::Intrinsics
{
    ...
    // ShiftLeftArithmetic
    template <typename T>
    constexpr T shal(T x, int s) noexcept
    {
        return shll(x, s);
    }
    ...
    // CountTrailingZeros
    template <typename T>
    constexpr int cntt0(T x) noexcept
    {
    ...
    // CountLeadingZeros
    template <typename T>
    constexpr int cntl0(T x) noexcept
    {
        constexpr int nbits = int(sizeof(T) * CHAR_BIT);
        if (x == 0)
        {
    ...
    // CountTrailingOnes
    template <typename T>
    constexpr int cntl1(T x) noexcept
    {
        return cntl0(~x);
    }
    // CountLeadingOnes
    template <typename T>
    constexpr int cntt1(T x) noexcept
    {
        return cntt0(~x);
    }
    // CountBits
    ...
    // ReverseBits
    template <typename T>
    constexpr auto reverse_bits(T x, int subword_bits = 1, int group_subwords = 1) noexcept
    ...
    template <typename T>
    constexpr T reverse_bytes(T x, int bytes_per_block = 1, int blocks_per_group = sizeof(T)) noexcept
    {
        return reverse_bits(x, CHAR_BIT * bytes_per_block, blocks_per_group);
    }

    template <typename T>
    constexpr T setbit(T x, int b) noexcept
    {
        return x | (T(1) << b);
    }

    template <typename T>
    constexpr T rstbit(T x, int b) noexcept
    {
        return x & ~(T(1) << b);
    }

    template <typename T>
    constexpr T flipbit(T x, int b) noexcept
    {
        return x ^ (T(1) << b);
    }

    template <typename T>
    constexpr bool testbit(T x, int b) noexcept
    {
        return x & (T(1) << b);
    }

    template <typename T>
    constexpr T rstbitsge(T x, int b) noexcept
    {
        return x & ((T(1) << b) - 1);
    }

    template <typename T>
    constexpr T rstbitsle(T x, int b) noexcept
    {
        return x & ~((T(1) << (b + 1)) - 1);
    }

    template <typename T>
    constexpr T setbitsge(T x, int b) noexcept
    {
        return x | ~((T(1) << b) - 1);
    }

    template <typename T>
    constexpr T setbitsle(T x, int b) noexcept
    {
        return x | ((T(1) << (b + 1)) - 1);
    }

    template <typename T>
    constexpr T flipbitsge(T x, int b) noexcept
    {
        return x ^ ~((T(1) << b) - 1);
    }

    template <typename T>
    constexpr T flipbitsle(T x, int b) noexcept
    {
        return x ^ ((T(1) << (b + 1)) - 1);
    }
    ...
    // CeilPowerOf2
    template <typename T>
    constexpr T ceilp2(T x) noexcept
    {
        x = x - 1;
        x |= shlr(x, 1);
        x |= shlr(x, 2);
    ...
    // FloorPowerOf2
    template <typename T>
    constexpr T floorp2(T x) noexcept
    {
        x |= shlr(x, 1);
        x |= shlr(x, 2);
        x |= shlr(x, 4);
    ...
    constexpr auto satadd(TL l, TR r) noexcept -> decltype(l + r)
    {
    ...
    template <typename TL, typename TR>
    constexpr auto satsub(TL l, TR r) noexcept -> decltype(l - r)
    {
        typedef decltype(l + r) LR;
    ...
    template <typename T>
    constexpr T outer_pshuffle(T x) noexcept
    {
        T t = 0;
        if (sizeof(T) > 4)
        {
    ...
    template <typename T>
    constexpr T outer_punshuffle(T x) noexcept
    {
        T t = 0;
        t   = (x ^ shlr(x, 1)) & T(0x2222222222222222UL);
        x   = x ^ t ^ shll(t, 1);
    ...
    template <typename T>
    constexpr T inner_pshuffle(T x) noexcept
    {
        return outer_pshuffle(reverse_bits(x, sizeof(T) * CHAR_BIT / 2, 2));
    }

    template <typename T>
    constexpr T inner_punshuffle(T x) noexcept
    {
...
{
    template <typename T>
    constexpr T unsigned_negate(T x) noexcept
    {
    ...
    template <typename T>
    constexpr T unsigned_not(T x) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const nx = ~x;
        return nx;
    ...
    template <typename T>
    constexpr T unsigned_extend(T x) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_nor(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const xy  = x | y;
        T const nxy = ~xy;
    ...
    template <typename T>
    constexpr T unsigned_andnot(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const ny  = ~y;
        T const xny = x & ny;
    ...
    template <typename T>
    constexpr T unsigned_and(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const xy = x & y;
        return xy;
    ...
    template <typename T>
    constexpr T unsigned_xor(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const xy = x ^ y;
        return xy;
    ...
    template <typename T>
    constexpr T unsigned_or(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const xy = x | y;
        return xy;
    ...
    template <typename T>
    constexpr T unsigned_or(T x, T y, T z) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        T const xyz = x | y | z;
        return xyz;
    ...
    template <typename T>
    constexpr T unsigned_select(T mask, T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_cmp_lt(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_cmp_ge(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_cmp_gt(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        using Ts = std::make_signed_t<T>;
    ...
    template <typename T>
    constexpr T unsigned_cmp_le(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_nez(T x) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        using Ts = std::make_signed_t<T>;
    ...
    template <typename T>
    constexpr T unsigned_nezp(T x) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_eqz(T x) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_cmp_eq(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
    ...
    template <typename T>
    constexpr T unsigned_cmp_neq(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        return unsigned_nez<T>(x - y);
    }
    ...
    template <typename T>
    constexpr T unsigned_srl(T x, T bits) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        return x >> bits;
    }
    ...
    template <typename T>
    constexpr T unsigned_sll(T x, T bits) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        return x << bits;
    }
    ...
    template <typename T>
    constexpr T unsigned_add(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        return x + y;
    }
    ...
    template <typename T>
    constexpr T unsigned_sub(T x, T y) noexcept
    {
        static_assert(std::is_unsigned_v<T>);
        return x - y;
    }
