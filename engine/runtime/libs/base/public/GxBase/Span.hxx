...
namespace notstd
{
    ...
    public:
        ...
        [[nodiscard]] constexpr size_type max_length() const noexcept
        {
        ...
        [[nodiscard]] constexpr span first(difference_type count) const noexcept
        {
        ...
        [[nodiscard]] constexpr span first() const noexcept
        {
            assert(static_cast<size_type>(Length) < m_Size);
            return span{ m_Data, static_cast<size_type>(Length) };
        };

        [[nodiscard]] constexpr span last(difference_type count) const noexcept
        {
            assert(static_cast<size_type>(count) < m_Size);
        ...
        template <difference_type Length>
        [[nodiscard]] constexpr span last() const noexcept
        {
            assert(static_cast<size_type>(Length) < m_Size);
        ...
        [[nodiscard]] constexpr span subspan() const noexcept
        {
            assert(static_cast<size_type>(Offset + Length) <= m_Size);
            return span{ m_Data + Offset, static_cast<size_type>(Length) };
        }

        [[nodiscard]] constexpr span subspan(difference_type offset, difference_type count) const noexcept
        {
    ...
    template <typename T>
    [[nodiscard]] constexpr span<const std::byte> as_bytes(span<T> other) noexcept
    {
