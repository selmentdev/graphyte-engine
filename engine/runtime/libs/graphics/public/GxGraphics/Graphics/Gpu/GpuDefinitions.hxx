...
namespace Graphyte::Graphics
{
    ...
    ///
    /// @note Essentially, these enum values represent internal shader format. For example,
    ...
    public:
        ...
        static constexpr GpuFilter MakeBasic(
            ...
            GpuFilterType mag,
            GpuFilterType mip,
            GpuFilterReductionType reduction) noexcept
        {
            uint16_t const result = static_cast<uint16_t>(
                ((static_cast<uint16_t>(min) & FILTER_TYPE_MASK) << FILTER_MIN_SHIFT) | ((static_cast<uint16_t>(mag) & FILTER_TYPE_MASK) << FILTER_MAG_SHIFT) | ((static_cast<uint16_t>(mip) & FILTER_TYPE_MASK) << FILTER_MIP_SHIFT) | ((static_cast<uint16_t>(reduction) & REDUCTION_TYPE_MASK) << REDUCTION_TYPE_SHIFT));
    ...
    public:
        ...
        void SetTexture(
            ...
            GpuShaderVisibility shader_visibility) noexcept
        {
    ...
    private:
        void Set(
            const GpuResourceBinding& binding) noexcept
        {
