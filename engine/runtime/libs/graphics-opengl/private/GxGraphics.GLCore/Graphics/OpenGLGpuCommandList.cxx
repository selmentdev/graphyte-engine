...
namespace Graphyte::Graphics
{
        ...
        GpuRenderTargetHandle handle) noexcept
    {
        ...
        GPU_GL_VALIDATE(glBindFramebuffer(GL_FRAMEBUFFER, native->m_Framebuffer));
    }
        ...
        GpuGraphicsPipelineStateHandle handle) noexcept
    {
        ...
        GPU_GL_VALIDATE(glBindVertexArray(native->m_VertexArrayObject));

        GPU_GL_VALIDATE(glUseProgram(native->m_Program));
        ...
        {
            GPU_GL_VALIDATE(glEnable(GL_CULL_FACE));

            GPU_GL_VALIDATE(glCullFace(native->m_RasterizerState.CullMode));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_CULL_FACE));
        }

        GPU_GL_VALIDATE(glPolygonMode(GL_FRONT_AND_BACK, native->m_RasterizerState.FillMode));
        ...
        {
            GPU_GL_VALIDATE(glEnable(GL_DEPTH_CLAMP));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_DEPTH_CLAMP));
        }

        if (native->m_RasterizerState.ScissorEnable != GL_FALSE)
        {
            GPU_GL_VALIDATE(glEnable(GL_SCISSOR_TEST));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_SCISSOR_TEST));
        }

        if (native->m_RasterizerState.MultisampleEnable != GL_FALSE)
        {
            GPU_GL_VALIDATE(glEnable(GL_MULTISAMPLE));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_MULTISAMPLE));
        }

        if (native->m_RasterizerState.AntialiasedLineEnable != GL_FALSE)
        {
            GPU_GL_VALIDATE(glEnable(GL_LINE_SMOOTH));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_LINE_SMOOTH));
        }

        GLenum front_face = (native->m_RasterizerState.FrontCounterClockwise != GL_FALSE)
                                ? GL_CCW
                                : GL_CW;

        GPU_GL_VALIDATE(glFrontFace(front_face));

        if (GLAD_GL_NV_conservative_raster)
        {
            if (native->m_RasterizerState.ConservativeRasterizationMode != GL_FALSE)
            {
                GPU_GL_VALIDATE(glEnable(GL_CONSERVATIVE_RASTERIZATION_NV));
            }
            else
            {
                GPU_GL_VALIDATE(glDisable(GL_CONSERVATIVE_RASTERIZATION_NV));
            }
        }
        ...
        {
            GPU_GL_VALIDATE(glEnable(GL_SAMPLE_ALPHA_TO_COVERAGE));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_SAMPLE_ALPHA_TO_COVERAGE));
        }
        ...
        for (size_t i = 0; i < std::size(native->m_BlendState.RenderTarget); ++i)
        {
            ...
            {
                GPU_GL_VALIDATE(glEnablei(GL_BLEND, index));
            ...
            {
                GPU_GL_VALIDATE(glDisablei(GL_BLEND, index));
            }
        }
        ...
        {
            GPU_GL_VALIDATE(glEnable(GL_DEPTH_TEST));

            GPU_GL_VALIDATE(glDepthMask(native->m_DepthStencilState.DepthWriteMask));

            GPU_GL_VALIDATE(glDepthFunc(native->m_DepthStencilState.DepthFunction));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_DEPTH_TEST));
        }

        if (native->m_DepthStencilState.StencilEnable != GL_FALSE)
        {
            GPU_GL_VALIDATE(glEnable(GL_STENCIL_TEST));
        ...
        {
            GPU_GL_VALIDATE(glDisable(GL_STENCIL_TEST));
        }

        if (native->m_DepthStencilState.DepthBoundsTestEnable != GL_FALSE)
        {
            GPU_GL_VALIDATE(glEnable(GL_DEPTH_BOUNDS_TEST_EXT));
        }
        else
        {
            GPU_GL_VALIDATE(glDisable(GL_DEPTH_BOUNDS_TEST_EXT));
        }


        ////////////

        GPU_GL_VALIDATE(glActiveTexture(GL_TEXTURE0 + 0));

        GPU_GL_VALIDATE(glActiveTexture(GL_TEXTURE0 + 1));

        GPU_GL_VALIDATE(glActiveTexture(GL_TEXTURE0 + 2));

        GPU_GL_VALIDATE(glActiveTexture(GL_TEXTURE0 + 3));
    }
